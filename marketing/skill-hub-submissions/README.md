...
| CLIHunt | https://clihunt.dev | Web form | Submitted for review | Submitted `Prevu` as a DevOps developer tool with repo URL, tagline, and `npm install -g @prevu/cli`; page confirmed `Thanks! Your tool has been submitted for review.` |
| ToolHunter | https://www.toolhunter.cc/submit | Web form | Blocked by CAPTCHA | Form filled with project name, repo URL, and preview-environment description, but final submit requires reCAPTCHA; do not bypass automated verification. |
| AgentDepot | https://agentdepot.dev/submit | Web form to email draft | Prepared, pending manual send | Completed the multi-step form as `Prevu Preview Environments`; site showed `Submission Ready!` and instructed sending through the email client. Do not send from a personal mailbox without account approval. |
| FindAIDir | https://www.findaidir.com/submit-tool | Web form | Blocked by category validator | Form filled with Prevu details, but `Category is invalid` persisted for visible categories such as `ai agents`, `Productivity`, and `Other`. |
...
| Launch AI Jam | https://launchaijam.com/submit | Auth-gated form | Blocked on account authorization | Submit page requires sign-in before any tool details can be entered; defer until a brand account is available. |
| productdirs | https://productdirs.com/projects/submit | Auth-gated form | Blocked on account authorization | Submit Project redirects to Google sign-in and email sign-in is disabled; defer until a brand Google account is available. |
